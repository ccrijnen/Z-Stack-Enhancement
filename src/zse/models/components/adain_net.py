...
class Decoder(nn.Sequential):
    def __init__(self, out_channels=1):
        super().__init__(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(64, out_channels, kernel_size=3, stride=1, padding=1, padding_mode='reflect'),
        )
...
class AdaINNet(nn.Module):
    ...
    def forward(self, content, style, alpha: float = 1.0, loss: bool = False):
        ...
        style_enc = style_features[self.content_layer]

        adain = self.adain(content_enc, style_enc)
        adain = alpha * adain + (1 - alpha) * content_enc

        out = self.decoder(adain)
